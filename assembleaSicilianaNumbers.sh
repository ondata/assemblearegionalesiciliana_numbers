...
# per ogni pagina di un deputato estraggo il numero di attività svolte da primo firmatario e da cofirmatario
  ...
  do
    ...
    curl -s "$uriDeputato" | scrape -be "//table[2]//tr[5]/td[3]/table//tr[2]/td" | xml2json | jq '[.html.body.td | if ."$t"|type == "string" then {idDeputato:'"$idDeputato"',idGruppo:'"$idGruppo"',DisegnidiLegge:."$t"|gsub("(\\[|\\])";"")} else {idDeputato:'"$idDeputato"',idGruppo:'"$idGruppo"',DisegnidiLegge:."$t"[0]|gsub("(\\[|\\])";""),InterrogazioniParlamentari:."$t"[1]|gsub("(\\[|\\])";""),InterpellanzeParlamentari:."$t"[2]|gsub("(\\[|\\])";""),Mozioni:."$t"[3]|gsub("(\\[|\\])";""),Ordinidelgiorno:."$t"[4]|gsub("(\\[|\\])";"")} end]' > PF_"$idDeputato".json
    curl -s "$uriDeputato" | scrape -be "//table[2]//tr[5]/td[3]/table//tr[5]/td" | xml2json | jq '[.html.body.td | if ."$t"|type == "string" then {idDeputato:'"$idDeputato"',DisegnidiLegge_co:."$t"|gsub("(\\[|\\])";""),uriDeputato:"'"$uriDeputato"'"} else {idDeputato:'"$idDeputato"',DisegnidiLegge_co:."$t"[0]|gsub("(\\[|\\])";""),InterrogazioniParlamentari_co:."$t"[1]|gsub("(\\[|\\])";""),InterpellanzeParlamentari_co:."$t"[2]|gsub("(\\[|\\])";""),Mozioni_co:."$t"[3]|gsub("(\\[|\\])";""),Ordinidelgiorno_co:."$t"[4]|gsub("(\\[|\\])";""),uriDeputato:"'"$uriDeputato"'"} end]'  > CF_"$idDeputato".json
  done
done
